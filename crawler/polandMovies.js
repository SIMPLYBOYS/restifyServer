...
var posterPages = [];
var releaseUrl = [];
var moviePages = [ //specific for poland movies
    3,
    4,
    5,
    4,
...
exports.polandMovies = function() {
    ...
    function (err) {
        if (err) console.error(err.stack);
          console.log('all jobs for poland movies update finished!!');
    });
};
        ...
        function (callback) {
            ...
            async.whilst(
                ...
                function (innercallback) {  
                    url = 'http://maoyan.com/films?sourceId=19&yearId='+(11-count)+'&offset='+(innerCount*30)+'&sortId=1';
                    request({
                        url: url,   
                        encoding: "utf8",
                        method: "GET"
                        ...
                        async.whilst(
                            ...
                            function(callback) {
                                dbPoland.poland.findOne({'title': title[thirdCount]}, function(err, doc){
                                    if (doc) {
                                        dbPoland.poland.update({'title': title[thirdCount]}, {'$set': {
...
function insertDetail(done) {
    var count = 0;
        ...
        async.whilst(
                ...
                function(callback) {
                    dbPoland.poland.findOne({'title': movieList[count]}, function(err, doc) {
                        if (doc) {                           
                            request({
                                url: doc['detailUrl'],
                                encoding: "utf8",
                                ...
                                dbPoland.poland.update({'title': movieList[count]}, {'$set': {
...
function insertTrailer(done) {
    ...
    async.whilst(
        ...
        function(callback) {
            dbPoland.poland.findOne({title: movieList[count]}, function(err, doc) {
                if (doc) {
                    new TrendsTrailer('pl', movieList[count], youTube, count, callback);
                    count++;
                } else {
                    count++;
                    callback(null, count);
        ...
        function(err, n) {
            console.log('insert pl Trailer finish ' + n);
            done(null);
        }
    ); 
}
...
function cleanData(done) {
    ...
    dbPoland.poland.find({}, function(err, docs) {  
        ...
        async.whilst(
            ...
            function(callback) {
                dbPoland.poland.remove({title: movieObj[count]['title']}, function(err, doc) {
                    if (!err) {
                        count++;
                        callback(null, count);
                    }
            ...
            function(err, n) {
                console.log('clean poland films finish ' + n);
                done(null);
            }
        );
    });
...
function createIndex(done) {
    ...
    dbPoland.poland.find({}, function(err, docs) {  
        ...
        async.whilst(
            ...
            function(err, n) {
                console.log('pl films indexing finish ' + n);
                done(null);
            }
        );
    });
