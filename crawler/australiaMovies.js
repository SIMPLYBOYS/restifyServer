...
var posterPages = [];
var releaseUrl = [];
var moviePages = [ //specific for australia movies
    11,
    17,
    16,
    17,
...
exports.australiaMovies = function() {
    async.series([
        insertTitle,
        insertDetail,
        insertTrailer,
    ...
    function (err) {
        if (err) console.error(err.stack);
          console.log('all jobs for australia movies update finished!!');
    });
};
        ...
        function (callback) {
            ...
            async.whilst(
                ...
                function (innercallback) {  
                    url = 'http://maoyan.com/films?sourceId=20&yearId='+(11-count)+'&offset='+(innerCount*30)+'&sortId=1';
                    request({
                        url: url,   
                        encoding: "utf8",
                        method: "GET"
                        ...
                        async.whilst(
                            ...
                            function(callback) {
                                dbAustralia.australia.findOne({'title': title[thirdCount]}, function(err, doc){
                                    if (doc) {
                                        dbAustralia.australia.update({'title': title[thirdCount]}, {'$set': {
...
function insertDetail(done) {
    var count = 0;
        ...
        async.whilst(
                ...
                function(callback) {
                    dbAustralia.australia.findOne({'title': movieList[count]}, function(err, doc) {
                        if (doc) {                           
                            request({
                                url: doc['detailUrl'],
                                encoding: "utf8",
                                ...
                                dbAustralia.australia.update({'title': movieList[count]}, {'$set': {
...
function insertTrailer(done) {
    ...
    async.whilst(
        ...
        function(callback) {
            dbAustralia.australia.findOne({title: movieList[count]}, function(err, doc) {
                if (doc) {
                    new TrendsTrailer('au', movieList[count], youTube, count, callback);
                    count++;
                } else {
                    count++;
                    callback(null, count);
        ...
        function(err, n) {
            console.log('insert au Trailer finish ' + n);
            done(null);
        }
    ); 
}
...
function cleanData(done) {
    var movieObj = [];
    dbAustralia.australia.find({}, function(err, docs) {  
        ...
        async.whilst(
            ...
            function(callback) {
                dbAustralia.australia.remove({title: movieObj[count]['title']}, function(err, doc) {
                    if (!err) {
                        count++;
                        callback(null, count);
                    }
            ...
            function(err, n) {
                console.log('clean australia films finish ' + n);
                done(null);
            }
        );
    });
...
function createIndex(done) {
    ...
    dbAustralia.australia.find({}, function(err, docs) {  
        ...
        async.whilst(
            ...
            function(err, n) {
                console.log('au films indexing finish ' + n);
                done(null);
            }
        );
    });
