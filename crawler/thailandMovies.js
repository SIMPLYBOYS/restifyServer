...
var posterPages = [];
var releaseUrl = [];
var moviePages = [ //specific for thailand movies
    1,
    4,
    4,
    4,
...
exports.thailandMovies = function() {
    async.series([
        insertTitle,
        insertDetail,
        insertTrailer,
    ...
    function (err) {
        if (err) console.error(err.stack);
          console.log('all jobs for tl movies update finished!!');
    });
};
        ...
        function (callback) {
            ...
            async.whilst(
                ...
                function (innercallback) {  
                    url = 'http://maoyan.com/films?sourceId=9&yearId='+(11-count)+'&offset='+(innerCount*30)+'&sortId=1';
                    request({
                        url: url,   
                        encoding: "utf8",
                        method: "GET"
                        ...
                        async.whilst(
                            ...
                            function(callback) {
                                dbThailand.thailand.findOne({'title': title[thirdCount]}, function(err, doc){
                                    if (doc) {
                                        dbThailand.thailand.update({'title': title[thirdCount]}, {'$set': {
...
function insertDetail(done) {
    var count = 0;
        ...
        async.whilst(
                ...
                function(callback) {
                    dbThailand.thailand.findOne({'title': movieList[count]}, function(err, doc) {
                        if (doc) {                           
                            request({
                                url: doc['detailUrl'],
                                encoding: "utf8",
                                ...
                                dbThailand.thailand.update({'title': movieList[count]}, {'$set': {
...
function insertTrailer(done) {
    ...
    async.whilst(
        ...
        function(callback) {
            dbThailand.thailand.findOne({title: movieList[count]}, function(err, doc) {
                if (doc) {
                    new TrendsTrailer('tl', movieList[count], youTube, count, callback);
                    count++;
                } else {
                    count++;
                    callback(null, count);
        ...
        function(err, n) {
            console.log('insert tl Trailer finish ' + n);
            done(null);
        }
    ); 
}
...
function cleanData(done) {
    var movieObj = [];
    dbThailand.thailand.find({}, function(err, docs) {  
        ...
        async.whilst(
            ...
            function(callback) {
                dbThailand.thailand.remove({title: movieObj[count]['title']}, function(err, doc) {
                    if (!err) {
                        count++;
                        callback(null, count);
                    }
            ...
            function(err, n) {
                console.log('clean thailand films finish ' + n);
                done(null);
            }
        );
    });
