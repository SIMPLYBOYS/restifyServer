...
var posterPages = [];
var releaseUrl = [];
var moviePages = [ //specific for italia movies
    9,
    18,
    17,
    18,
...
exports.italiaMovies = function() {
    ...
    function (err) {
        if (err) console.error(err.stack);
          console.log('all jobs for italia movies update finished!!');
    });
};
        ...
        function (callback) {
            ...
            async.whilst(
                ...
                function (innercallback) {  
                    url = 'http://maoyan.com/films?sourceId=16&yearId='+(11-count)+'&offset='+(innerCount*30)+'&sortId=1';
                    request({
                        url: url,   
                        encoding: "utf8",
                        method: "GET"
                        ...
                        async.whilst(
                            ...
                            function(callback) {
                                dbItalia.italia.findOne({'title': title[thirdCount]}, function(err, doc){
                                    if (doc) {
                                        dbItalia.italia.update({'title': title[thirdCount]}, {'$set': {
...
function insertDetail(done) {
    var count = 0;
        ...
        async.whilst(
                ...
                function(callback) {
                    dbItalia.italia.findOne({'title': movieList[count]}, function(err, doc) {
                        if (doc) {                           
                            request({
                                url: doc['detailUrl'],
                                encoding: "utf8",
                                ...
                                dbItalia.italia.update({'title': movieList[count]}, {'$set': {
...
function insertTrailer(done) {
    ...
    async.whilst(
        ...
        function(callback) {
            dbItalia.italia.findOne({title: movieList[count]}, function(err, doc) {
                if (doc) {
                    new TrendsTrailer('it', movieList[count], youTube, count, callback);
                    count++;
                } else {
                    count++;
                    callback(null, count);
        ...
        function(err, n) {
            console.log('insert it Trailer finish ' + n);
            done(null);
        }
    ); 
}
...
function cleanData(done) {
    var movieObj = [];
    dbItalia.italia.find({}, function(err, docs) {  
        ...
        async.whilst(
            ...
            function(callback) {
                dbItalia.italia.remove({title: movieObj[count]['title']}, function(err, doc) {
                    if (!err) {
                        count++;
                        callback(null, count);
                    }
            ...
            function(err, n) {
                console.log('clean italia films finish ' + n);
                done(null);
            }
        );
    });
...
function createIndex(done) {
    ...
    dbItalia.italia.find({}, function(err, docs) {  
        ...
        async.whilst(
            ...
            function(err, n) {
                console.log('it films indexing finish ' + n);
                done(null);
            }
        );
    });
