...
var posterPages = [];
var releaseUrl = [];
var moviePages = [ //specific for india movies
    21,
    29,
    28,
    27,
...
exports.indiaMovies = function() {
    ...
    function (err) {
        if (err) console.error(err.stack);
          console.log('all jobs for india movies update finished!!');
    });
};
        ...
        function (callback) {
            ...
            async.whilst(
                ...
                function (innercallback) {  
                    url = 'http://maoyan.com/films?sourceId=8&yearId='+(11-count)+'&offset='+(innerCount*30)+'&sortId=1';
                    request({
                        url: url,   
                        encoding: "utf8",
                        method: "GET"
                        ...
                        async.whilst(
                            ...
                            function(callback) {
                                dbIndia.india.findOne({'title': title[thirdCount]}, function(err, doc){
                                    if (doc) {
                                        dbIndia.india.update({'title': title[thirdCount]}, {'$set': {
...
function insertDetail(done) {
    var count = 0;
        ...
        async.whilst(
                ...
                function(callback) {
                    dbIndia.india.findOne({'title': movieList[count]}, function(err, doc) {
                        if (doc) {                           
                            request({
                                url: doc['detailUrl'],
                                encoding: "utf8",
                                ...
                                dbIndia.india.update({'title': movieList[count]}, {'$set': {
...
function insertTrailer(done) {
    ...
    async.whilst(
        ...
        function(callback) {
            dbIndia.india.findOne({title: movieList[count]}, function(err, doc) {
                if (doc) {
                    new TrendsTrailer('id', movieList[count], youTube, count, callback);
                    count++;
                } else {
                    count++;
                    callback(null, count);
        ...
        function(err, n) {
            console.log('insert id Trailer finish ' + n);
            done(null);
        }
    ); 
}
...
function cleanData(done) {
    var movieObj = [];
    dbIndia.india.find({}, function(err, docs) {  
        ...
        async.whilst(
            ...
            function(callback) {
                dbIndia.india.remove({title: movieObj[count]['title']}, function(err, doc) {
                    if (!err) {
                        count++;
                        callback(null, count);
                    }
            ...
            function(err, n) {
                console.log('clean india films finish ' + n);
                done(null);
            }
        );
    });
...
function createIndex(done) {
    ...
    dbIndia.india.find({}, function(err, docs) {  
        ...
        async.whilst(
            ...
            function(err, n) {
                console.log('id films indexing finish ' + n);
                done(null);
            }
        );
    });
