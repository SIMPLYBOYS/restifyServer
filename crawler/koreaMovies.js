...
var posterPages = [];
var releaseUrl = [];
var moviePages = [ //specific for korea movies
    11,
    14,
    14,
    13,
...
exports.koreaMovies = function() {
    ...
    function (err) {
        if (err) console.error(err.stack);
          console.log('all jobs for korea movies update finished!!');
    });
};
        ...
        function (callback) {
            ...
            async.whilst(
                ...
                function (innercallback) {  
                    url = 'http://maoyan.com/films?sourceId=7&yearId='+(11-count)+'&offset='+(innerCount*30)+'&sortId=1';
                    request({
                        url: url,   
                        encoding: "utf8",
                        method: "GET"
                        ...
                        async.whilst(
                            ...
                            function(callback) {
                                dbKorea.korea.findOne({'title': title[thirdCount]}, function(err, doc){
                                    if (doc) {
                                        dbKorea.korea.update({'title': title[thirdCount]}, {'$set': {
...
function insertDetail(done) {
    var count = 0;
        ...
        async.whilst(
                ...
                function(callback) {
                    dbKorea.korea.findOne({'title': movieList[count]}, function(err, doc) {
                        if (doc) {                           
                            request({
                                url: doc['detailUrl'],
                                encoding: "utf8",
                                ...
                                dbKorea.korea.update({'title': movieList[count]}, {'$set': {
...
function insertTrailer(done) {
    ...
    async.whilst(
        ...
        function(callback) {
            dbKorea.korea.findOne({title: movieList[count]}, function(err, doc) {
                if (doc) {
                    new TrendsTrailer('kr', movieList[count], youTube, count, callback);
                    count++;
                } else {
                    count++;
                    callback(null, count);
...
function cleanData(done) {
    var movieObj = [];
    dbKorea.korea.find({}, function(err, docs) {  
        ...
        async.whilst(
            ...
            function(callback) {
                dbKorea.korea.remove({title: movieObj[count]['title']}, function(err, doc) {
                    if (!err) {
                        count++;
                        callback(null, count);
                    }
...
function createIndex(done) {
    ...
    dbKorea.korea.find({}, function(err, docs) {  
        ...
        async.whilst(
            ...
            function(err, n) {
                console.log('kr films indexing finish ' + n);
                done(null);
            }
        );
    });
