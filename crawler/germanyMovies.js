...
var posterPages = [];
var releaseUrl = [];
var moviePages = [ //specific for germany movies
    18,
    31,
    31,
    44,
...
exports.germanyMovies = function() {
    async.series([
        insertTitle,
        insertDetail,
        insertTrailer,
    ...
    function (err) {
        if (err) console.error(err.stack);
          console.log('all jobs for gm movies update finished!!');
    });
};
        ...
        function (callback) {
            ...
            async.whilst(
                ...
                function (innercallback) {  
                    url = 'http://maoyan.com/films?sourceId=11&yearId='+(11-count)+'&offset='+(innerCount*30)+'&sortId=1';
                    request({
                        url: url,   
                        encoding: "utf8",
                        method: "GET"
                        ...
                        async.whilst(
                            ...
                            function(callback) {
                                dbGermany.germany.findOne({'title': title[thirdCount]}, function(err, doc){
                                    if (doc) {
                                        dbGermany.germany.update({'title': title[thirdCount]}, {'$set': {
...
function insertDetail(done) {
    var count = 0;
        ...
        async.whilst(
                ...
                function(callback) {
                    dbGermany.germany.findOne({'title': movieList[count]}, function(err, doc) {
                        if (doc) {                           
                            request({
                                url: doc['detailUrl'],
                                encoding: "utf8",
                                ...
                                dbGermany.germany.update({'title': movieList[count]}, {'$set': {
...
function insertTrailer(done) {
    ...
    async.whilst(
        ...
        function(callback) {
            dbGermany.germany.findOne({title: movieList[count]}, function(err, doc) {
                if (doc) {
                    new TrendsTrailer('gm', movieList[count], youTube, count, callback);
                    count++;
                } else {
                    count++;
                    callback(null, count);
...
function cleanData(done) {
    var movieObj = [];
    dbGermany.germany.find({}, function(err, docs) {  
        ...
        async.whilst(
            ...
            function(callback) {
                dbGermany.germany.remove({title: movieObj[count]['title']}, function(err, doc) {
                    if (!err) {
                        count++;
                        callback(null, count);
                    }
            ...
            function(err, n) {
                console.log('clean germany films finish ' + n);
                done(null);
            }
        );
    });
...
function createIndex(done) {
    ...
    dbGermany.germany.find({}, function(err, docs) {  
        ...
        async.whilst(
            ...
            function(err, n) {
                console.log('gr films indexing finish ' + n);
                done(null);
            }
        );
    });
