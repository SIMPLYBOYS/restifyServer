                ...
                function (innercallback) {  
                    ...
                    request({
                        ...
                        async.whilst(
                            ...
                            function(callback) {
                                dbChina.china.findOne({'title': title[thirdCount]}, function(err, doc){
                                    if (doc) {
                                        dbChina.china.update({'title': title[thirdCount]}, {'$set': {
...
function insertDetail(done) {
    var count = 0;
        ...
        async.whilst(
                ...
                function(callback) {
                    dbChina.china.findOne({'title': movieList[count]}, function(err, doc) {
                        if (doc) {                           
                            request({
                                url: doc['detailUrl'],
                                encoding: "utf8",
...
function insertTrailer(done) {
    ...
    async.whilst(
        ...
        function(callback) {
            dbChina.china.findOne({title: movieList[count]}, function(err, doc) {
                if (doc) {
                    new TrendsTrailer('cn', movieList[count], youTube, count, callback);
                    count++;
                } else {
                    count++;
                    callback(null, count);
...
function createIndex(done) {
    ...
    dbChina.china.find({}, function(err, docs) {  
        ...
        async.whilst(
            ...
            function(err, n) {
                console.log('cn films indexing finish ' + n);
                done(null);
            }
        );
    });
