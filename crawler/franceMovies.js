...
var moviePages = [ //specific for france movies
    19,
    31,
    35,
    37,
...
exports.franceMovies = function() {
    ...
    function (err) {
        if (err) console.error(err.stack);
          console.log('all jobs for france movies update finished!!');
    });
};
        ...
        function (callback) {
            ...
            async.whilst(
                ...
                function (innercallback) {  
                    url = 'http://maoyan.com/films?sourceId=4&yearId='+(11-count)+'&offset='+(innerCount*30)+'&sortId=1';
                    request({
                        url: url,   
                        encoding: "utf8",
                        method: "GET"
                        ...
                        async.whilst(
                            ...
                            function(callback) {
                                dbFrance.france.findOne({'title': title[thirdCount]}, function(err, doc){
                                    if (doc) {
                                        dbFrance.france.update({'title': title[thirdCount]}, {'$set': {
...
function insertDetail(done) {
    var count = 0;
        ...
        async.whilst(
                ...
                function(callback) {
                    dbFrance.france.findOne({'title': movieList[count]}, function(err, doc) {
                        if (doc) {                           
                            request({
                                url: doc['detailUrl'],
                                encoding: "utf8",
                                ...
                                dbFrance.france.update({'title': movieList[count]}, {'$set': {
...
function insertTrailer(done) {
    ...
    async.whilst(
        ...
        function(callback) {
            dbFrance.france.findOne({title: movieList[count]}, function(err, doc) {
                if (doc) {
                    new TrendsTrailer('fr', movieList[count], youTube, count, callback);
                    count++;
                } else {
                    count++;
                    callback(null, count);
        ...
        function(err, n) {
            console.log('insert fr Trailer finish ' + n);
            done(null);
        }
    ); 
}
...
function cleanData(done) {
    var movieObj = [];
    dbFrance.france.find({}, function(err, docs) {  
        ...
        async.whilst(
            ...
            function(callback) {
                dbFrance.france.remove({title: movieObj[count]['title']}, function(err, doc) {
                    if (!err) {
                        count++;
                        callback(null, count);
                    }
...
function createIndex(done) {
    ...
    dbFrance.france.find({}, function(err, docs) {  
        ...
        async.whilst(
            ...
            function(err, n) {
                console.log('fr films indexing finish ' + n);
                done(null);
            }
        );
    });
