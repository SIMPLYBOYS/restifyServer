...
var posterPages = [];
var releaseUrl = [];
var moviePages = [ //specific for honkong movies
    2,
    3,
    3,
    3,
...
exports.honkongMovies = function() {
    async.series([
        insertTitle,
        insertDetail,
        insertTrailer,
        ...
        function (callback) {
            ...
            async.whilst(
                ...
                function (innercallback) {  
                    url = 'http://maoyan.com/films?sourceId=10&yearId='+(11-count)+'&offset='+(innerCount*30)+'&sortId=1';
                    request({
                        url: url,   
                        encoding: "utf8",
                        method: "GET"
                        ...
                        async.whilst(
                            ...
                            function(callback) {
                                dbHonKong.honkong.findOne({'title': title[thirdCount]}, function(err, doc){
                                    if (doc) {
                                        dbHonKong.honkong.update({'title': title[thirdCount]}, {'$set': {
...
function insertDetail(done) {
    var count = 0;
        ...
        async.whilst(
                ...
                function(callback) {
                    dbHonKong.honkong.findOne({'title': movieList[count]}, function(err, doc) {
                        if (doc) {                           
                            request({
                                url: doc['detailUrl'],
                                encoding: "utf8",
                                ...
                                dbHonKong.honkong.update({'title': movieList[count]}, {'$set': {
...
function insertTrailer(done) {
    ...
    async.whilst(
        ...
        function(callback) {
            dbHonKong.honkong.findOne({title: movieList[count]}, function(err, doc) {
                if (doc) {
                    new TrendsTrailer('hk', movieList[count], youTube, count, callback);
                    count++;
                } else {
                    count++;
                    callback(null, count);
        ...
        function(err, n) {
            console.log('insert hk Trailer finish ' + n);
            done(null);
        }
    ); 
}
...
function cleanData(done) {
    ...
    dbHonKong.honkong.find({}, function(err, docs) {  
        ...
        async.whilst(
            ...
            function(callback) {
                dbHonKong.honkong.remove({title: movieObj[count]['title']}, function(err, doc) {
                    if (!err) {
                        count++;
                        callback(null, count);
                    }
