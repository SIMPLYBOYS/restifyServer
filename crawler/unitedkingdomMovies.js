...
var moviePages = [ //specific for uk movies
    48,
    76,
    78,
    73,
...
exports.ukMovies = function() {
    ...
    function (err) {
        if (err) console.error(err.stack);
          console.log('all jobs for uk movies update finished!!');
    });
};
        ...
        function (callback) {
            ...
            async.whilst(
                ...
                function (innercallback) {  
                    url = 'http://maoyan.com/films?sourceId=5&yearId='+(11-count)+'&offset='+(innerCount*30)+'&sortId=1';
                    request({
                        url: url,   
                        encoding: "utf8",
                        method: "GET"
                        ...
                        async.whilst(
                            ...
                            function(callback) {
                                dbUK.uk.findOne({'title': title[thirdCount]}, function(err, doc){
                                    if (doc) {
                                        dbUK.uk.update({'title': title[thirdCount]}, {'$set': {
...
function insertDetail(done) {
    var count = 0;
        ...
        async.whilst(
                ...
                function(callback) {
                    dbUK.uk.findOne({'title': movieList[count]}, function(err, doc) {
                        if (doc) {                           
                            request({
                                url: doc['detailUrl'],
                                encoding: "utf8",
                                ...
                                if ($('.award-list') != null) {
                                    if ($('.award-list').length == undefined) {
                                        ...
                                        $('.award-list').each(function(index, item) {
                                            if (index == 1) {
                                                $('.award-list .award-item').each(function(index, item) {
                                                   console.log($(item).text().trim().split(' ')[0]);
                                                   console.log($(item).find('.content').text().trim());
                                ...
                                dbUK.uk.update({'title': movieList[count]}, {'$set': {
...
function insertTrailer(done) {
    ...
    async.whilst(
        ...
        function(callback) {
            dbUK.uk.findOne({title: movieList[count]}, function(err, doc) {
                if (doc) {
                    new TrendsTrailer('uk', movieList[count], youTube, count, callback);
                    count++;
                } else {
                    count++;
                    callback(null, count);
        ...
        function(err, n) {
            console.log('insert uk Trailer finish ' + n);
            done(null);
        }
    ); 
}
...
function cleanData(done) {
    ...
    dbUK.uk.find({}, function(err, docs) {  
        ...
        async.whilst(
            function() { return count < movieObj.length},
            function(callback) {
                dbUK.uk.remove({title: movieObj[count]['title']}, function(err, doc) {
                    if (!err) {
                        count++;
                        callback(null, count);
                    }
            ...
            function(err, n) {
                console.log('clean unitedkingdom films finish ' + n);
                done(null);
            }
        );
    });
